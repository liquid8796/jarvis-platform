# Architecture decisions · 1.0.76

## 1.0.76 interactive cancellation and durable session handles (current)

Interactive desktop ownership now follows the remote call lifetime for both `browser` and `computer` categories. `ToolExecutionResources.ReleaseCallLeaseOnCancellation(...)` keeps this policy explicit: the acquired call-owned resource reference is wrapped in `CancellationBoundResourceLease`, whose cancellation callback queues release outside the token callback. This closes the `computer.request_access` gap where a timed-out grant/permission dialog could continue unwinding while still holding the shared `desktop` resource and leave a subsequent browser action queued. Filesystem, shell and process leases retain their existing completion/retention behavior, and retained process-job references are unaffected.

Application-session handles are correlation capabilities rather than standalone authorization, so their lifetime now follows the underlying Agent session instead of an absolute wall-clock expiry. New handles carry claims version 2 and no `ExpiresAt`; resolution continues to require valid data protection, authenticated owner equality, enrolled-device equality and a valid `js_...` session identity before the Agent performs its own open/closed session check. The server deliberately accepts legacy claims version 1 without enforcing the old expiry timestamp so an existing open session can resume after the former 30-day boundary. OAuth/access-token lifetime, device authorization, Arm/Pause, local tool approval, per-call deadlines and explicit session close remain separate enforcement boundaries.

## 1.0.75 browser companion build closure
...
At 1.0.64, the local SQLite session registry keyed records by owner/device/session, snapshotted the workspace at admission, kept optimistic revisions, persisted bounded cursor mailboxes, and retained closed-session tombstones longer than the then-current 30-day protected handle lifetime. Version 1.0.76 supersedes only that wall-clock handle cutoff; Agent context still validates ownership before approval/execution, data-protection keys must remain persistent across upgrades, and account/device mismatches, closed sessions and missing required handles fail closed.
...
## Codex-compatible tool consolidation (1.0.89)

The Windows agent registry exposes five mutation/inspection primitives: `source.apply_patch`, `image.view_image`, `unified_exec.exec_command`, `unified_exec.write_stdin`, and `computer_use.computer_use`. Legacy filesystem-write, shell, process-job, and public computer helper descriptors are no longer included in `AgentHello.Tools` or `catalog.changed`.

`apply_patch` performs workspace-relative V4A Add/Update/Move/Delete operations with optimistic session-read validation and rollback. `exec_command` owns the process and returns a numeric `session_id` when it outlives the initial yield; `write_stdin` polls, writes, or sends Ctrl+C while enforcing owner/device/session identity. `computer_use` resolves native windows, snapshots UI Automation state, and executes window-relative actions while retaining the existing local application-grant gate. Internal access-request and application-launch helpers are implementation details and are not advertised as public MCP tools.

## Authoritative catalog lifecycle (1.0.90)

`Device.CapabilitiesJson` remains the persisted per-device capability snapshot, while `Tools` stores optional publication policy and public metadata. `ToolCatalogReconciler` computes the union of all enrolled manifests under a process-wide serial lock, filters the central retired-ID set, adds missing `Auto` rows, refreshes live metadata for `Auto`, and deletes duplicate or no-longer-advertised policy rows. Reconciliation runs before HTTP serving and after Agent/device/user lifecycle changes, so the admin catalog is not an append-only history.

Selected-device execution still resolves directly from that device's manifest. The same retired-ID filter is applied before MCP listing, generic search/call and task-plan validation, so an older Agent cannot revive a server-retired surface. Retirement is explicit for compatibility: when a future release intentionally removes a canonical ID while older Agents may still report it, that ID must be added to `AgentToolCatalogRules`; ordinary dynamic removals need no code change because absence from every persisted manifest is sufficient.