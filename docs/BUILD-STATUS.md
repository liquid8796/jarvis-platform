...
- The separate real Chromium suite passed **17/17** on Agent service **1.0.78**, extension **1.4.0** and Chromium **151**, retaining **14 actual PNGs**. It exercises production SessionBrowserToolSet, BrowserRuntimeClient RPC, BrowserServiceServer, vendor tools, shipping extension and real CDP/scripting. It detects broken CTA, arbitrary console error, post-action HTTP 500, obstruction/disabled state, clipped mobile target and hidden parent frame; delayed hydration, shadow DOM, same-origin frame, per-tab references and session ownership pass. Report: `artifacts/browser-live/2026-09-19T11-14-26-836Z-94d10fa8/results.json`.
- Browser contract/session JavaScript tests passed **15/15**; Python publish-asset checks passed **8/8**. Package/assembly version verification passed **1.0.78 / 1.0.78.0**. The publish verifier now requires the new `Assets/Browser/qa.js` alongside the manifest/background runtime.
- The offline release build produced Windows x64 Desktop/CLI ZIP and Linux ARM64 server tarball. Build used `-SkipTests` only after the independently executed suites above. Release verification checks archive CRC/hash parity, prohibited runtime/private files, dependency layout, DLL versions and published CLI help; Linux binaries are inspected but not executed on this Windows machine.
- The live harness replaces only native-port transport registration with a private test relay; it does not claim OS native-host registry/stdio launch coverage, live production MCP acceptance, model visual-judgment quality or identical Codex behavior. Cross-origin interactions with unverifiable coordinates fail explicitly. Local source hashes bind evidence freshness, not arbitrary remote server build identity.
- Final live-run cleanup passed and its owned processes/profile were removed. Earlier interrupted/failed harness attempts retained test-only temporary profiles after cleanup rejection or file-lock errors; those are excluded from acceptance and were not used as a fallback.
- Source publication/build does not replace the running Agent or restart the deployed MCP server. Deploy server/Agent 1.0.78, install/reload extension 1.4.0 and refresh MCP tool discovery to activate the new workflow.
