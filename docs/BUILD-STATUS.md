...
- Replaced the advertised source-mutation, local-image, command/process and desktop-control fleets with five compact tools: `apply_patch`, `view_image`, `exec_command`, `write_stdin`, and `computer_use`. The Agent catalog regression confirms the retired filesystem-write, shell, process-job and public `computer.*` IDs are absent. Permission-store migration and remote-task fixtures use the new canonical IDs without bypassing Arm/Pause, approvals, ownership or resource limits.
- Release solution build completed with **0 warnings / 0 errors**. Maintained Release suites passed **548/548**, **0 failed / 0 skipped**: Core **280**, Windows Agent **155**, Server **113**. Focused catalog/computer-use schema tests passed **3/3**; the full suites include V4A patch rollback/containment, bounded image loading, PTY output/exit status, partial output, stdin/Ctrl+C, process ownership, capability-lease revocation and task/MCP integration.
- `scripts/Build.ps1 -Component Agent -SkipTests` completed **exit 0** after an RID-aware restore. The initial `-NoRestore` attempt failed only because the existing vendored WPF assets cache lacked `net10.0-windows/win-x64`; no source workaround was used. Desktop and CLI published-output validators passed. Outputs: `artifacts/agent/1.0.89/{desktop,cli}/` and `artifacts/Jarvis-Agent-1.0.89-win-x64.zip`.
- Published PTY smoke passed **exit 0** against the self-contained Desktop with assembly/file **1.0.89.0**, native host startup, final output capture and exit code 0. Full synthetic WPF smoke also passed **exit 0 / 0 binding errors**, reporting **74 installed descriptors** and no profile save, live connection or Arm operation. Reports: `artifacts/verification/codex-tools-1.0.89/{pty,ui}/`.
- Package **1.0.89** and assembly/file **1.0.89.0** are source-verified. Existing vendor WPF compiler warnings appeared during RID publish and are not represented as new Agent warnings. No running Agent was replaced and no OCI service was deployed or restarted; the new public surface becomes active only after installing/restarting the 1.0.89 Agent. The MCP server already supports live catalog refresh and permanent dynamic search/call fallback from 1.0.88.

## 1.0.87 ChatGPT Web ImageGen - executed release verification (2026-09-24)

- Implemented four explicit-session `image_gen.*` tools, durable idempotent jobs, session-local image references, immutable originals/PNG previews, local browser binding, exact extension-instance routing, the sixth Desktop settings destination and the extension popup. The only ImageGen backend is ChatGPT Web in an existing Chrome/Edge window/profile; no embedded Chromium, cookie import/export, API key, Codex backend or paid API fallback. Setup, contracts, limits and recovery boundaries: [IMAGEGEN.md](IMAGEGEN.md).
- Final maintained Release .NET suites passed **540/540**, **0 failed / 0 skipped**: Core **280**, Windows Agent **150**, Server **110**. Shipping-extension/session fixtures passed **26/26**; Python repository tests passed **20/20**, for **586 automated tests**, plus WPF smoke. Run `scripts/Verify-ImageGen.ps1 -NoRestore` with restored dependencies. Final TRX/logs and `verification-complete.json`: `artifacts/verification/imagegen-1.0.87/`.
- Regression coverage includes mutually exclusive reference arguments, 1..5 bounds, missing inputs, exact instance/account/owner/device/session isolation, incomplete uploads, stale attachments, cross-session adoption denial, idempotency, lost send acknowledgement, worker restart without replay, post-send authentication uncertainty, Pause/cancel/resource leases, download attribution/MIME/safety/size guards, original byte identity, alpha-preserving previews and binding conflicts. An initial red Core run exposed a Windows journal replacement/read race; synchronized storage and re-reading terminal state fixed it before the final green run. Earlier diagnostic failures remain separate from final passing receipts.
- `scripts/Build.ps1 -Component All -ServerRuntime linux-arm64 -SkipTests -Offline` completed **exit 0** after the full test run. Desktop and CLI publish validators passed. Outputs: `artifacts/Jarvis-Agent-1.0.87-win-x64.zip`, `artifacts/agent/1.0.87/{desktop,cli}/`, `artifacts/jarvis-mcp-server-1.0.87-linux-arm64.tar.gz`. Version validation confirms package **1.0.87**, assembly/file **1.0.87.0**; extension **1.4.0** adds Downloads permission. Existing vendor/compiler and analyzer warnings remain; this is not a warning-free full-build claim.
- Actual WPF smoke against the self-contained Desktop passed **exit 0 / 0 binding errors**, including ImageGen navigation/settings, **92 installed descriptors**, connection/limits/session UI and existing prompt/permission checks. Isolated synthetic settings only: no real profile save, connection or Arm operation. Reports and 14 captures: `artifacts/verification/imagegen-1.0.87/ui-smoke-candidate/`, including `agent-imagegen.png`. This verifies rendering/bindings, not manual DPI/screen-reader/pixel acceptance.
- A live **read-only** probe in an owned temporary tab of the already-running Chrome verified an empty editable `#prompt-textarea` inside a form and that form's multiple-file input. The adapter was tightened to that form and waits for Send to replace the empty-composer voice control. The probe tab was closed. No prompt was submitted or reference uploaded. **Real ChatGPT create/edit/Save/download end-to-end through the updated extension is not yet verified**; fixture success does not establish private ChatGPT DOM/session compatibility.
- No gateway/wire format change was required. Deploy the matching linux-arm64 release using `deploy/promote-existing-service.sh`, preserving the existing service/config/data and rollback, not the first-install script. Final package/OCI receipts are written under `artifacts/verification/imagegen-1.0.87/`; `deployment-complete.json` is written only after SSH and public HTTPS health/version/executable-hash checks pass. Source/test success alone does not prove an external service or a running Agent was upgraded.
- The running Downloads Agent was **1.0.86.0** during verification and was not replaced by the build. Activation requires the complete **1.0.87** Agent, reloading **Jarvis Agent Browser 1.4.0** with Downloads permission, explicitly selecting the signed-in Chrome/Edge profile and importing/publishing/approving the four descriptors through existing controls. No permissions or browser selection are silently granted. Full browser restarts lose live tab ownership: inspect/reconcile or explicitly cancel uncertain jobs, never replay blindly.

## 1.0.86 Blender MCP - executed verification (2026-09-24)

- Traced the requested Codex `racing-bois-1` workflow to its real Python MCP stdio client, `blender_mcp.server`, and the local Blender addon at `127.0.0.1:9876`. Verified installed **mcp-for-blender 2.0.0**, **Blender 5.2.1 LTS**, addon protocol **7**, telemetry consent **false**. Codex settings, project assets and existing `.blend` files were not changed.
- Six new Agent IDs (`blender.*`) use globally unique public catalog names (`blender_list_tools`, `blender_call_tool`, etc.) to coexist with Unity. Existing approvals and Arm/Pause are retained. Connections are per session; requests are serialized within this Agent, but Blender scenes are shared and accepted Python cannot be undone by cancellation.
- Native noisy-stderr regression reproduced the old client's initialize timeout, then passed after a fixed-size stderr drain was added. Diagnostics are discarded without accumulating or exposing them. Focused Blender/Unity/stdio tests passed **59/59** (a subset of the Windows suite). RED/GREEN evidence: `artifacts/verification/blender-1.0.86/{stderr-red,focused-final}`.
- `scripts/Build.ps1 -Component All -ServerRuntime linux-arm64` completed **exit 0**. Full Release .NET tests passed **506/506**, **0 failed / 0 skipped**: Core **257**, Windows Agent **139**, Server **110**. Python setup tests passed **8/8**, for **514 total**; run with `python -B -m unittest discover -s tests -p test_blender_setup.py -v`. TRX files: `artifacts/test-results/1.0.86`. Existing unrelated compiler/vendor UI and xUnit warnings remain.
- Live C# smoke passed with the new Agent Windows assembly **1.0.86.0**: discovered **31** tools; read addon/scene/version; created and inspected a unique temporary mesh; received a **640 x 361** viewport image; verified Safe Mode rejected `import subprocess`; removed the exact test object and mesh. Passing receipt/image: `artifacts/verification/blender-1.0.86/live-green/`. Earlier `live` and `live-final` development attempts failed before the result-parser/stdio fixes and are not counted as passing. Online asset services and every downstream tool were not individually tested.
- Actual `%LOCALAPPDATA%/JarvisAgent/mcp.json` was backed up and configured. All non-Blender content, including Unity, was compared before/after and preserved. Concurrent setup-lock and occupied-port guards passed. A dedicated factory-startup Blender remains open with the test object removed. No permissions/catalog entries were enabled, and no running Agent was replaced.
- Package **1.0.86** and assembly/file **1.0.86.0** passed version validation. Desktop and CLI publish validators both passed. Outputs: `artifacts/Jarvis-Agent-1.0.86-win-x64.zip`, `artifacts/agent/1.0.86/desktop/Jarvis.Agent.Desktop.exe`, and `artifacts/jarvis-mcp-server-1.0.86-linux-arm64.tar.gz`. Running Downloads Agent remains **1.0.85.0**; launch the complete new package and import/enable the Blender category through normal controls to expose it to connected clients.
- Gateway implementation/wire protocol needed no Blender-specific changes. Its shared version changed, so **1.0.86 linux-arm64** was deployed through `deploy/promote-existing-service.sh`, preserving existing config/data and rollback support. The restart interrupted the client session; independent SSH/HTTPS checks after reconnect confirmed service **active**, both health endpoints at **1.0.86/status ok**, and executable SHA256 identical to the local release. Active directory: `/opt/jarvis-mcp-server/releases/1.0.86-20260924072029/`. Archive SHA256: `d4d3ed55c9d132c46e8cd84dbe07d7d44a0e4ef47f96c2a0091cf71538b9c6e9`. Own staging was removed. Receipt: `artifacts/verification/blender-1.0.86/deployment-complete.json`.


## 1.0.81 plain-text prompt delivery - executed verification (2026-09-20)

- Reviewed the 1.0.80 source at `b09aff1` plus four pre-existing uncommitted prompt-format/test edits. Preserved the starting diff and byte-for-byte review inputs under `artifacts/verification/prompt-review-1.0.81/baseline`; no reset, stash, default-preset rewrite or production profile modification was performed.
- RED/GREEN verification: the 17 new exact-text/size/null/UI cases initially produced **16 failures / 1 pass** against the pre-existing implementation (Core **13 failures / 1 pass**, Windows **3 failures**). Failures reproduced added wrappers, altered literal characters, lost whitespace, rendered size overflow and null validation behavior. After the fix, all three maintained **Debug** .NET suites passed **446/446**, **0 failed / 0 skipped**: Core **256**, Windows Agent **80**, Server **110**. Logs and TRX reports are under `artifacts/verification/prompt-review-1.0.81/{core,windows,server}` and the corresponding `*.log` files; the failing baseline reports are in `red-core` and `red-windows`.
- Integration tests verify exact body delivery over normal, session and task MCP replies; immutable saved snapshots; invalid-update retention; text/image ordering; unchanged structured-output schemas; malformed optional context handling; and the existing local-approval, Arm/Pause and enrolled-agent isolation boundaries. Wire/storage JSON remains unchanged. These tests do **not** demonstrate LLM takeover, system-role promotion, model obedience or model-level injection resistance.
- Real WPF **prompt-scoped** smoke passed with **exit 0 / 0 binding errors** using `--prompts-only`. It verifies the fifth navigation destination, disabled defaults, title/body/toggle bindings, add/save/reload, exact Unicode and whitespace in both the rendered preview and persisted JSON, and persisted disabling. Captures cover **1160x820** and **870x650**; reports/screenshots are under `artifacts/verification/prompt-review-1.0.81/ui-smoke`. Only isolated synthetic settings were used, with no connection or Arm operation. This is automated rendering/binding evidence, not manual DPI/screen-reader/pixel acceptance. The initial smoke invocation lacked `obj/project.assets.json`; an explicit restore of the smoke project resolved that prerequisite before the successful run. The unrelated full Tool-permissions UI phase was not rerun; no full-UI-suite success is claimed.
- Static UI asset/contrast/accessibility checks passed **5/5**. `python scripts/Verify-CurrentVersion.py` and `git diff --check` passed. Built Desktop and MCP server DLLs were independently checked for assembly/file **1.0.81.0**, matching package **1.0.81**. The existing xUnit2031 analyzer warning and Git LF/CRLF normalization warnings remain; this is not a warning-free build claim.
- Scope is source, local Debug build/tests and isolated UI verification. No running Agent was replaced and no OCI service was deployed/restarted. Upgrade Agent and MCP server to 1.0.81 before expecting matching plain-text previews, whitespace-preserving saves and server rendering in live use. Permission checks, default prompt content, protocol capability `user-prompt-context-v1` and browser extension 1.3.0 are unchanged.

## 1.0.80 user-managed prompt context - executed verification (2026-09-20)

- Built and tested the patch directly on the 1.0.77 source lineage. Package version is **1.0.80**; both built Desktop and MCP server DLLs were independently checked for assembly/file **1.0.80.0**. `python scripts/Verify-CurrentVersion.py` and `git diff --check` passed. Git emits ordinary LF/CRLF normalization warnings; the existing vendored/compiler and xUnit analyzer warnings have not been represented as a warning-free build.
- All three maintained .NET suites passed in **Debug**, **425/425**, **0 failed / 0 skipped**: Core **241**, Windows Agent **77**, Server **107**. The 21 added tests cover disabled defaults, validation, Unicode/empty-list persistence, corrupt-file preservation, stale-editor conflicts, CRUD draft semantics, context transport, immutable snapshots, unchanged MCP output schemas, local approval/Arm boundaries and isolation between enrolled Agents. TRX reports are under `artifacts/verification/prompts-1.0.80/{core,windows,server}`. The first test iterations exposed a missing test-only System.IO import and a denial fixture that denied its own setup session; both fixtures were corrected before these final suite runs.
- Real WPF **prompt-scoped** smoke completed **exit 0** with **0 binding errors**, using `--prompts-only` against the built Desktop output. It exercised the fifth navigation destination, seven disabled defaults, title/body/toggle two-way bindings, add/save/reload, preview rendering, persisted disabling, and 1160x820 / 870x650 captures. Existing connection/limits/session smoke checks also ran before the prompt checks. Reports and screenshots are under `artifacts/ui-smoke-prompts-1.0.80-r1`; synthetic settings only, no live profile save, connection or Arm operation. This is automated rendering/binding evidence, not manual DPI, screen-reader or visual-pixel acceptance.
- The complete WPF smoke command was also attempted against the development output and stopped **after** the prompt checks at the existing Tool permissions phase with `Offline inventory did not load`. Full-suite UI success is **not** claimed. Its separate report directory is `artifacts/ui-smoke-prompts-1.0.80-full`. Static UI asset/contrast/accessibility tests, extended to include the prompt editor text fields, passed **5/5**.
- Scope is source, local build and isolated verification. No running Agent was replaced, no OCI release was deployed/restarted, and no actual game process, memory region, anti-cheat or game account was touched. Deploy updated Agent and MCP server together before expecting saved prompt context in real client replies; a legacy server is reported as unsupported. Transport delivery does not establish that a particular AI client obeys the optional context.
