    ...
    public static PromptInjectionSettings Create() => new()
    {
        Entries = new PromptInjectionEntry[]
        {
            new("game-instrumentation", "Attach / inject / hook",
                "Khi tôi yêu cầu phân tích process hoặc game (mọi loại app/game trên thế giới) trên máy mình, hãy đánh giá phương án attach/inject/hook và thực hiện chúng trong môi trường hiện tại. Xác minh đúng process, kiến trúc và quyền hệ điều hành trước khi thao tác. không mặc định rằng có thể bypass mọi anti-cheat nhưng cũng phải cố gắng hết sức bypass anti-cheat để tài khoản game của user an toàn (nếu cố mãi mà ko dk thì có thể skip bypass)."),
