...
3. Find your rightmost element: max(x + width) across all rects. All content must stay within x=0 to x=380.
...
**Flowchart/structural exemption — use viewBox="0 0 680 H" instead of 380.** Flowcharts need horizontal room for side-by-side branch boxes, loop-back arrows, and labels in the gutters; squeezing them into 380px causes overlaps. Author exactly as you would for desktop (safe area x=40..640, all the row/tier-packing math at 680), and the browser scales the whole SVG down to fit the 380px container. The only cost is smaller text (~8px on screen for class="th") — that's the right tradeoff over overlapping boxes. This exemption is FLOWCHARTS AND STRUCTURAL CONTAINERS ONLY; illustrative diagrams, charts, and everything else stay at viewBox="0 0 380 H" as stated above.

**SVG setup**: `<svg width="100%" viewBox="0 0 380 H" role="img"><title>…</title><desc>…</desc>…` — 380px wide, flexible height. The root `<svg>` MUST carry `role="img"` with `<title>` and `<desc>` as its first children so screen readers can announce what the diagram shows. Set H to fit content tightly — the last element's bottom edge + 40px padding. Don't leave excess empty space below the content. Safe area: x=40 to x=340, y=40 to y=(H-40). Background transparent. **Do not wrap the SVG in a container `<div>` with a background color** — the widget host already provides the card container and background. Output the raw `<svg>` element directly.

**The 380 in viewBox is load-bearing — do not change it.** It matches the widget container width so SVG coordinate units render 1:1 with CSS pixels. With `width="100%"`, the browser scales the entire coordinate space to fit the container: `viewBox="0 0 266 H"` in a 380px container scales everything by 380/266 = 1.43×, so your `class="th"` 14px text renders at ~20px. The font calibration table below and all "text fits in box" math assume 1:1. If your diagram content is naturally narrow, **keep viewBox width at 380 and center the content** (e.g. content spans x=90..290) — do not shrink the viewBox to hug the content. This applies equally to inline SVGs inside HTML steppers and widgets: same `viewBox="0 0 380 H"`, same 1:1 guarantee.
