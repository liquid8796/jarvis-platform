...
**Layout**: Prefer single-direction flows (all top-down or all left-right). Keep diagrams simple — max 4-5 nodes per diagram. The widget is narrow (~380px) so complex layouts break.
...
- Max 2-3 nesting levels. Deeper nesting gets unreadable at 380px width.
...
- **Layout follows the subject's geometry**, not a grid. If the thing is tall and narrow (a water heater, a thermometer), the diagram is tall and narrow. If it's wide and flat (a PCB, a geological cross-section), the diagram is wide. Let the subject dictate proportions within the 380px viewBox width.
...
All core rules still apply (viewBox 380px, dark mode mandatory, 14/12px text, pre-built classes, arrow marker, clickable nodes).
...
**Illustrative diagrams at 380px**: the full interactive worked example (water-heater cross-section with thermostat slider, animated convection, and right-margin callouts) is omitted here — it was authored at 680px and would need a complete re-layout, not a coordinate scale, to fit. Follow the composition rules above with `viewBox="0 0 380 H"` directly: lay the subject tall-and-narrow at e.g. x=40..220, reserve x=230..360 for right-side leader-line labels (`class="ts"`, `text-anchor="start"`), and stack controls below the SVG instead of beside it.

For abstract subjects (e.g. transformer attention) at 380px: same approach. The five-token fan-out example is omitted — at this width, lay out three tokens (x≈60, 160, 260; w≈80 each) with the middle one as the query, fan three weight lines, and put the caption below at y past the lowest stroke.
