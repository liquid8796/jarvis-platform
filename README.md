# Jarvis Control - 1.0.81

**Version 1.0.81 completes plain-text prompt delivery.** Saved, enabled preset bodies are appended exactly as entered in one separate MCP tool-result text block, in saved order with blank-line separators: no added source banner, nested JSON, title prefix, fence markers or invisible character rewriting. Preview, save/reload and delivery now preserve body whitespace and Unicode, and rendered size includes separators. Titles remain editor metadata. This is tool context, not a system/developer message or an authorization mechanism. Agent/server wire JSON and local JSON storage remain compatible with `user-prompt-context-v1`. See the [operator guide](docs/AGENT.md) for scope, privacy and delivery limits, and [BUILD-STATUS.md](docs/BUILD-STATUS.md) for executed checks. Build/push does not itself update a running Agent or production MCP service.
